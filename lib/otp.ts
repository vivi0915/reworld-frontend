...
export function smsConfiguration() {
  ...
  if (!config.SMS || !config.OTP_SECRET || config.OTP_SECRET.length < 32) throw new ApiError(503, '手機驗證暫時無法使用，你仍可建立 PLAYER 並保存此裝置的進度。');
...
export async function otpSendLimit(request: Request, phone: string, userId?: string) {
  const now = Date.now();
  const window = Math.floor(now / 3600000);
  const ip = request.headers.get('cf-connecting-ip') ?? 'local';
  const keys = await Promise.all([digest(`otp:cooldown:${phone}`), digest(`otp:phone:${phone}:${window}`), digest(`otp:ip:${ip}:${window}`), digest(`otp:session:${userId ?? ip}:${window}`)]);
  const db = database();
  ...
  if (results[1].results[0].attempts > 5 || results[2].results[0].attempts > 20 || results[3].results[0].attempts > 10) throw new ApiError(429, '發送次數過多，請稍後再試。');
